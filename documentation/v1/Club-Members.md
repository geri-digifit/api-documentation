...
|---|---|---|---|
|GET|/api/v1/club/`<club_id>`/member|Returns all member of the club that have been edited since `<sync_from>` (See *STRUCTURE*)|sync_from (in milliseconds), from_id, api_key, club_secret, club_member_id (optional), with (optional), any_sub_club (optional), rfid_tag (optional), external_id (optional), email (optional)|
...
|POST|/api/v1/club/`<club_id>`/member|Creates and returns a new member (See *STRUCTURE*)|api_key, club_secret|
...
| early_booking_access | boolean |yes| | If the member has early booking access                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
...
| ba_place                    |string|yes| | The name of the bank                                                                                                                                                                                                                                                                                   |
...
| unsubscribe_date            |string|yes| | The future inactive date of the member (YYYY-MM-DD). Can be removed by setting it to 0000-00-00                                                                                                                                                                                                                                                    
| early_booking_access        |boolean|yes| | Apply different limits for regular and early-access members, configurable per activity. This setting takes precedence when no memberships are assigned to the member. If a membership is assigned, early booking access depends on whether it’s included in their membership.                                                                                       |
...
|420|Invalid unsubscribe_date cannot be in the past|Occurs when you pass an unsubscribe date in the past|
